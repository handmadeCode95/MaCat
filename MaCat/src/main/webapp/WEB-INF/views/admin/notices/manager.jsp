...
<html lang="ko">
	<head>
		...
		<!-- 공지사항 관리자 테이블 전용 css-->
		<link rel="stylesheet"
			href="resources/css/admin/notices/admin_table.css">
...
<body>
	...
	<section class="wrap">
		...
		<form id="searchForm" action="products_search.mcat" method="post">
			<div class="notice_information_container">
				<!--회원정보-->
				<div class="notices_info">
					<div class="notices_info_title">
						<span>공지사항 정보</span>
					</div>
					<div class="infomation_control_part">
						<!--글번호 제목 작성일-->
						<div class="notices_info_middle">
							<div class="nots_chk_div">
								<div class="checks">
									<input type="checkbox" id="not_sq_chk" class="search" value="not_sq">
									<label	 for="not_sq_chk">글번호</label>
								</div>
								<div class="checks">
									<input type="checkbox" id="not_sj_chk" class="search" value="not_sj">
									<label for="not_sj_chk">제 목</label>
								</div>
								<div class="checks">
									<input type="checkbox" id="not_reg_dt" class="search" value="not_reg_dt">
									<label for="not_reg_dt">작성일</label>
								</div>
							</div>
							<div class="nots_input_div">
							...
							<!--기간선택 div-->
							<div class="nots_reg_dt_div">
								<span class="inputClickListener"><input type="date" class="not_reg_dt" name="not_reg_dt_start" disabled></span>
								 ~ 
								<span class="inputClickListener"><input type="date" class="not_reg_dt" name="not_reg_dt_end" disabled></span>
							</div>
						</div>
					</div>
				</div>
					...
					<table class="type11" id="checkbox_js">
						...
						<tbody id="searchResult">
							<c:forEach var="i" items="${notsDTO}">
								<tr id="${i.not_sq}">
									<td class="checks">
									<input name="nots" class="chkbox" type="checkbox" id="table_chk" value="${i.not_sq}"><label for="table_chk"></label>
									</td>
									<td><input name="not_sq" class="${i.not_sq}" type="hidden" value="${i.not_sq}" disabled>${i.not_sq}</td>
									<%-- <td><input name="not_sj" class="${i.not_sq}" type="text" value="${i.not_sj}" disabled></td> --%>
									<td>${i.not_sj}</td>
